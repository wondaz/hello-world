...
namespace Frame.Core
{
    public class ParamQuery
    {
        ...
        public ParamQuery AndWhere(string column, object value, Func<WhereData, string> cp = null,
            params object[] extend)
        {
            data.Where.Add(new ParamWhere
            {
                Data = new WhereData {AndOr = "and", Column = column, Value = value, Extend = extend},
                Compare = cp ?? Cp.Equal
            });
            return this;
        }
        ...
        public ParamQuery OrderBy(string sql)
        {
            ...
            if (!string.IsNullOrWhiteSpace(sql))
            {
                ...
                {
                    if (x.ToUpper().Trim().EndsWith("." + sortOrder[0].ToUpper())) sortOrder[0] = x;
                ...
                if (sortOrder[0].IndexOf(".", StringComparison.Ordinal) == -1 && !string.IsNullOrEmpty(mainTable))
