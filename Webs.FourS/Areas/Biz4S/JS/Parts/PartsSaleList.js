...
vms.search = function (vdata) {
    ...
    this.addClick = function () {
        com.openTab('新增备件销售订单', self.urls.edit);
    };
    //跳转到编辑页面
    this.editClick = function () {
        ...
        com.openTab('修改备件销售订单', self.urls.edit + row[self.pkey]);
    };
    ...
    this.outStockClick = function () {
        ...
        com.openTab('备件销售出库单', self.urls.outStock + row["SellOrderCode"]);
    };
    ...
    this.auditClick = function (vm, event) {
        var row = self.grid.datagrid('getSelected');
        if (!row) return com.message('warning', '请选择一条记录！');
        if (row.BillState == "2") return com.message('warning', '备件已出库，不能审核！');
        ...
        com.ajax({
            ...
            data: ko.toJSON({ code: row[self.pkey], status: btnStatus }),
            success: function (d) {
                if (d > 0) {
                    self.searchClick();
