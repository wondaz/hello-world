...
vms.search = function (vdata) {
    ...
    this.addClick = function () {
        ...
        addCarPayItem(row, self.searchClick);
        return true;
    };
};

var addCarPayItem = function (row, callBack) {
    com.dialog({
        title: "新增收款单",
        width: 550,
        height: 300,
        html: "#addCarPayItem-template",
        viewModel: function (w) {
            ...
            com.ajax({
                ...
                success: function (d) {
                    ...
                    thisWin.wform.Amount(row.FeeTotal);//总金额
                    thisWin.wform.SumPaid(row.SumPaid);//已收款
                    thisWin.wform.PayMoney(row.Debt);//欠款
                    thisWin.wform.CustomerID(row.CustomerID);//客户ID
                    thisWin.wform.PaymentType('整车付款');
                    thisWin.wform.VIN(row.VIN);
                    thisWin.wform.OriBillCode(row.SaleOrderCode);
            ...
            this.confirmClick = function () {
                ...
                if (thisWin.wform.PayMoney() > debt) {
                    return com.message('warning', '收款金额超出了！');
                }
