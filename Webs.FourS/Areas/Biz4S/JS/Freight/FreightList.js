...
vms.search = function (vdata) {
    ...
    this.editClick = function () {
        ...
        com.openTab('运费索赔单', self.urls.edit + row[self.pkey]);
        return true;
    };
    ...
    this.deleteClick = function () {
        var row = self.grid.datagrid('getSelected');
        if (!row) return com.message('warning', '请选择一条记录！');
        if (row.F_Status !== 0 && row.F_Status !== 3) return com.message('warning', '单据已在处理中，不能删除！');
    ...
    this.auditClick = function (vm, event) {
        var row = self.grid.datagrid('getSelected');
        if (!row) return com.message('warning', '请选择一条记录！');
        if (row.F_Status !== 2) return com.message('warning', '单据已在处理中，不能撤销！');

        com.message('confirm', '确定撤销提交吗？', function (b) {
            if (b) {
                com.ajax({
                    type: 'POST',
                    url: vdata.urls.revoke,
                    data: ko.toJSON({ id: row[self.pkey] }),
                    success: function (d) {
                        if (d === 'ok') {
                            self.searchClick();
