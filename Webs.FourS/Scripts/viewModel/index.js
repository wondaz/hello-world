...
wrapper.settings = {
    homeTabTitle: '我的桌面',
    homeTabUrl: '/sys4s/home/dashboardview',
    maxTabCount: 10
};

//初始化
wrapper.init = function () {
    com.ajax({ type: 'GET', url: 'api/sys4s/menu', success: wrapper.initMenu });
    ...
    com.dialog({
        ...
        viewModel: function (w) {
            w.find("#pwd_confirm").click(function () {
                if (!com.formValidate(w.find("#modifyPsd"))) return;

                if (w.find("#newPsd").val() != w.find("#confirmPsd").val()) {
                    com.message('warning', '新密码输入的不一致！');
                    return;
                }

                com.ajax({
                    url: '/api/sys4s/user/EditPassword',
                    async: false,
                    data: JSON.stringify({ "oldPsd": w.find("#oldPsd").val(), "newPsd": w.find("#newPsd").val() }),
                    success: function (d) {
                        if (!d) {
                            com.message('warning', '原密码错误,修改密码失败！');
                            return;
                        }

                        com.message('success', '修改密码成功！');
                        w.dialog('close');
                    }
                });
            });
...
wrapper.mysettings = function () {
    wrapper.addTab("个人设置", "/sys4s/config", "icon icon-wrench_orange");
};
...
wrapper.addTab = function (subtitle, url, icon) {
    ...
    if (wrapper.settings.isdialog == '1') {
        $('#dlg').dialog({
            title: subtitle,
            iconCls: icon,
            width: '100%',
    ...
    if (hasTab) {
        var existTab = $tab.tabs('getTab', subtitle);
        var src = $(existTab.panel('options').content).attr('src');
        hasTab = src == url;
    }
    ...
    var tabs = $tab.tabs('tabs');
    if (tabs.length > wrapper.settings.maxTabCount) {
        var firstTab = tabs[1].panel('options').title;
        $tab.tabs('close', firstTab);
    }
};
