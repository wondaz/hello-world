...
(function ($) {
    ...
    function setValuesHandle(target, value, queryText) {
        ...
        if(that.combo('getText')) return;

        var options = that.data('lookup').options;        
        if (options.url) {
            var queryParams = {};
            ...
            $.getJSON(options.url, queryParams, function (d) {
                var text = $.map(d.rows || d, function (v) { return v[options.textField] }).join(',');                
    ...
    function initlookup(target) {
        ...
        var fnShow = function () {
            ...
            options.window.content = "<iframe id='frm_win_" + options.valueField + "' src='" + options.view + (options.view.indexOf('?') > -1 ? '&' : '?') + "r=" + Math.random() + "' style='height:100%;width:100%;border:0;' frameborder='0'></iframe>";
        ...
        if (!options.customShowPanel && options.lookupType) {    //防止访问url,参数不对，产生错误，自定义的不需要这个url
            options.view = '/sys4s/plugins/lookup';
            options.url = '/sys4s/plugins/getlookupdata';
