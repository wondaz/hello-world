(function ($) {
    ...
    */
    ...
    $.fn.daterange = function (settings) {
        $(this).each(function () {
            _dateRangePicker.apply(this,[settings]);
        });
    };
    var _dateRangePicker = function (settings) {
        var rangeInput = $(this);
        ...
        var options = $.extend({
            presetRanges: [
				...
				{ text: '月初到今天', dateStart: function () { return Date.parse('today').moveToFirstDayOfMonth(); }, dateEnd: 'today' },
            //{text: '年初到今天', dateStart: function(){ var x= Date.parse('today'); x.setMonth(0); x.setDate(1); return x; }, dateEnd: 'today' },
            //extras:
            ...
            rangeSplitter: '到', //string to use between dates in single input
        ...
        var zh_CN = {
            ...
            firstDay: 1,
            isRTL: false,
            showMonthAfterYear: true,   //显示日历控件的时候：true年份在前月份在后，false相反
            //yearSuffix: '年',         //显示日历控件的时候：在年份后面加上年，有可能造成错位
            changeMonth: false,          //可以更改日期控件的月份
            changeYear: false,           //可以更改日期控件的年份
        ...
        //    inputName = rangeInput.attr("name"), fromCp = rangeInput.attr("from-data-cp") || "dtgreaterequal", toCp = rangeInput.attr("to-data-cp") || 'dtlessequal';
        ...
        function clickActions(el, rp, rpPickers, doneBtn) {
            ...
            if (el.is('.ui-daterangepicker-specificDate')) {
                ...
                rp.find('.range-start').saveDateToData().datepicker('setDate', null).css('opacity', 0).hide(400);
                ...
                rp.find('.range-end').saveDateToData().datepicker('setDate', null).css('opacity', 0).hide(400);
                setTimeout(function () { doneBtn.fadeIn(); }, 400);
            }
